...
export const Receipt = (() => {
  ...
  const transform = async (markdown, printer) => {
      ...
      const res = [];
      for (const line of markdown.normalize().split(/\n|\r\n|\r/)) {
  ...
  const barcode = (() => {
      ...
      const code128 = symbol => {
          const r = {};
          const s = symbol.data.replace(/((?!^[\x00-\x7f]+$).)*/, '');
          if (s.length > 0) {
              // generate HRI
              r.hri = symbol.hri;
      ...
      const codabar = symbol => {
          const r = {};
          const s = symbol.data.replace(/((?!^[A-D][0-9\-$:/.+]+[A-D]$).)*/i, '');
          if (s.length > 0) {
              // generate HRI
              r.hri = symbol.hri;
              r.text = s;
      ...
      const itf = symbol => {
          const r = {};
          const s = symbol.data.replace(/((?!^(\d{2})+$).)*/, '');
          if (s.length > 0) {
              // generate HRI
              r.hri = symbol.hri;
              r.text = s;
  ...
  return {
      ...
      prepareCommand(command: string) {
        return command.split('').map(c => `0${c.charCodeAt(0).toString(16)}`.slice(-2)).join(' ')
      }
  };

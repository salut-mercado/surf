  ...
  const handleSend = () => {
    ...
    try {
          ...
          if (response.data.items && Array.isArray(response.data.items)) {
            ...
            if (items.length === 0) {
              ...
              items.forEach((item: SupplierItem, index: number) => {
                ...
                botResponse += `${index + 1}. <strong>${
                  item.name
                }</strong> (code: ${item.code})<br>`;
            ...
            if (response.data.id) {
              ...
              if (response.data.name)
                botResponse += `name: ${response.data.name}<br>`;
              if (response.data.code)
                botResponse += `code: ${response.data.code}<br>`;
              if (response.data.agent)
                botResponse += `agent: ${response.data.agent}<br>`;
              if (response.data.phone)
                botResponse += `phone: ${response.data.phone}<br>`;
              if (response.data.taxID)
                botResponse += `taxID: ${response.data.taxID}<br>`;
              if (response.data.comments)
                botResponse += `comments: ${response.data.comments}<br>`;
              if (response.data.delayDays !== undefined)
                botResponse += `delayDays: ${response.data.delayDays}<br>`;
              if (response.data.blocked !== undefined) {
                const status = response.data.blocked
                  ? "🚫 blocked: true"
                  : "✅ blocked: false";
                botResponse += `blocked: ${status}<br>`;
              }
            ...
            botResponse =
              "Received response from server but format not recognized";
          }
      ...
      setMessages((prev) => [
        ...
        {
          role: "Bot",
          text: "Извините, произошла ошибка при отправке запроса.",
        },
      ]);
      setIsBotTyping(false);
    }
            ...
            >
              <div
                style={{
                  ...
                  fontSize: 15,
                  margin:
                    msg.role === "User" ? "2px 0 2px 40px" : "2px 40px 2px 0",
                  boxShadow:
                    msg.role === "User"
